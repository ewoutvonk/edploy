...
module Dployify
	...
	class Preferences
		...
		PREFERENCES_FILE = File.join(Dir.home, '.dployify.cfg')
		...
		def add_preset
			...
			puts "Note: A project name will be defined during the actual dployify run later on.\n"
			puts "Structure: <USERNAME>@<HOSTNAME>:<BASEPATH>/{PROJECT}.git"
			puts "Example: git@github.com:ewoutvonk/dploy.git"
		...
		def preferences
			unless @preferences
				...
				$dployify_cfg = File.open(PREFERENCES_FILE, "r+")
				at_exit do
					$dployify_cfg.close unless $dployify_cfg.closed?
				end
				preferences_contents = $dployify_cfg.read
		...
		def write_file
			$dployify_cfg.write YAML::dump(preferences)
			$dployify_cfg.truncate $dployify_cfg.pos
			$dployify_cfg.rewind
		end
