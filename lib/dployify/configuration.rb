...
module Dployify
	...
	class Configuration
		...
		def config
			unless @config
				dploy_yml = File.join(::RAILS_ROOT, 'config', 'dploy', 'dploy.yml')
				FileUtils.touch dploy_yml
				$dploy_yml = File.open(dploy_yml, "r+")
				at_exit do
					$dploy_yml.close unless $dploy_yml.closed?
				end
				config_contents = $dploy_yml.read
		...
		def write_file
			$dploy_yml.write YAML::dump(config)
			$dploy_yml.truncate $dploy_yml.pos
			$dploy_yml.rewind
		end
