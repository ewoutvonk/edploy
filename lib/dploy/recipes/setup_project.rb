...
namespace :dploy do
  ...
  after 'deploy:setup', 'dploy:setup_project'
  task :setup_project do
    ...
    Dploy::FileHandling.iterate_dir_items(extract_path) do |dirname, basename, relative_entry, fqpn|
      Dploy::FileHandling.upload_file(fqpn, "#{shared_path}/tmp") do |tmp_file|
        target_dir = dirname.gsub(/HOME/, '~')
        Dploy::Archive.extract tmp_file, target_dir, :remove => true
      end
    end
    
    Dploy::FileHandling.iterate_dir_items(init_scripts_path) do |dirname, basename, relative_entry, fqpn|
      Dploy::FileHandling.upload_file(fqpn, "#{shared_path}/tmp") do |tmp_file|
